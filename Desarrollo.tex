...
El prototipo fue diseñado en el software Proteus, desde el esquemático hasta la placa de circuito impreso (PCB), en la figura \ref{fig:esp32} se observa el esquematico de la tarjeta ESP32 construido en Proteus junto con su distribución de pines, además de sus conexiones correspondientes dentro del hardware. El prototipo está separado en dos secciones, la etapa de potencia AC y la etapa DC, en la última, se encuentra la mayor parte de circuitos que funcionan con corriente directa.\\
...
En los siguientes ítems se resaltaran las características más importantes que lleva el circuito:\\
	...
	\subsubsection{Calibración de audio:}
		Para calibrar la salida audible se hace uso de una resistencia variable (Potenciometro).\\
	...
	\subsubsection{Botón reset:}
		Presionando el botón reset se reinician las credenciales ingresadas posteriormente para la conexión del ESP32 a la red wifi.\\
	...
	\subsubsection{Etapa de potencia AC:}
		...
		Las dos cargas restantes corresponden a un sistema de encendido y apagado, cuyo funcionamiento se basa en un relevador SRA-05VDC-CL activado a 5V por medio de un transistor BJT como switch, este relevador tiene capacidad de 20A para cargas de 200W cada una, en la figura \ref{fig:ONOFAC} se observa el circuito diseñado en proteus. Para proteger el ESP32 el prototipo se vale del relevador, puesto que presenta un aislamiento magnético por la naturaleza de su funcionamiento.
		...
		Dentro de la etapa AC se encuentra el detector de cruce por cero, el cual se vale de un fototransistor 4N25, debido a su alta capacidad de aislamiento, soportando más de 2500VAC, tomando la onda rectificada completa y pasándola a un nivel lógico de 5V, esta parte del circuito se observa en la figura \ref{fig:DC01}; para que la señal sea más confiable se hace uso de un Schmitt-Trigger CD40106 mostrado en la figura \ref{fig:DC02}, valiéndose de la histéresis de voltaje para garantizar que su señal de salida sea poco susceptible al ruido \cite{DC0}.\\
	...
	\subsubsection{Etapa DC:}
		La etapa DC cuenta con cuatro salidas de control, de las cuales, dos de ellas están diseñadas con enfoque a motores, puesto que está equipada con control de velocidad a base de PWM e inversión de giro con un puente h usando transistores mosfet IRLZ44N, los cuales soportan una corriente de hasta 47A; este puente h está controlado por un circuito integrado L293D, que garantiza un voltaje Vgs adecuado para la correcta activación del los transistores del puente h; este circuito se muestra en la figura \ref{fig:L293D} y \ref{fig:CDC}.\cite{IRL}.\\
...
\begin{comment}	\subsection{Corrección de errores:}
	Durante la construcción del prototipo se corrigieron algunos fallos y errores que se presentaron, en esta sección se muestran estos contratiempos.\\
	
	\subsubsection{Creación de dispositivos en proteus:}
		 Durante la construcción del prototipo en el software proteus, fue necesaria la creación de dispositivos o componentes dentro del software, tanto la apariencia en el esquemático como el package en el diseño del PCB; durante la creación del package, se enumeraron dos pines incorrectos para dos dispositivos, los conversores del nivel DC-DC y los Triacs. Este error fue detectado al realizar la revisión del PCB una vez cumplió su proceso de construcción, por ende fue corregido sobre el prototipo y sobre el diseño.\\
		 
	\subsubsection{Sensores:}
		Al definir los pines correspondientes para los sensores conectados a la tarjeta ESP32, se cometió el error de conectar el sensor de calidad de aire (el cual presenta salida analógica de 0 a 5V) a uno de los pines correspondientes al ADC2 de la tarjeta, el cual no puede ser utilizado mientras que se utiliza la función de wifi, por ello se corrigió sobre el prototipo y sobre el diseño, dejando esa entrada de sensor en el pin GPIO34, correspondiente al ADC1 de la tarjeta.\\
		
	\subsubsection{Boton enable:}
		El circuito cerrado por el botón enable conecta el pin EN de la tarjeta a 3.3V debido a una confusión en el diseño, por ello se realizaron las correcciones en el prototipo y en el diseño, dejando el circuito cerrando con tierra.\\
		
	\subsubsection{Puente h en etapa DC:}
		Al momento de seleccionar el transistor IRLZ44N, se pasó por alto el hecho de que el transistor presenta un voltaje umbral entre la compuerta y la fuente (Vgs), es decir, el voltaje necesario para activar el transistor usado como switch, esta entre 1V y 2V \cite{IRL}; este hecho representa un problema, pues la activación de cada par de transistores se realizaba con una compuerta lógica negadora 74ls04, poniendo un estado contrario al otro par de transistores. Esta negadora suministraba 5V a la compuerta del transistor, lo cual no permite la activación de los transistores, por esto decidió reemplazar la compuerta lógica por el puente h L293D, de esta manera, activar cada par de transistores con un voltaje en la compuerta de 12V como salida el circuito integrado L293D. Este cambio fue realizado en el prototipo y en el diseño.\\
		\end{comment}
